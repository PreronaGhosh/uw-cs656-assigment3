...
def install_udp_middlebox_flow( source_dpid
    ...
    nw_graph = get_networkx_topology_graph()
    src_to_midbox = get_shortest_path_between(nw_graph, source_dpid, middlebox_dpid)
    midbox_to_dst = get_shortest_path_between(nw_graph, middlebox_dpid, destination_dpid)
    ...
    for sw in range(len(src_to_midbox)):
        ...
        if sw != len(src_to_midbox)-1:
            src_out_port, dst_in_port = get_ports_connecting(src_to_midbox[sw], src_to_midbox[sw+1])
            match_action = build_openflow_flowmod(match_fields, src_out_port)

        else:  # last switch in the path to middlebox, middlebox host on the other side
            port_to_host = get_host_port(src_to_midbox[sw])
            match_action = build_openflow_flowmod(match_fields, port_to_host)

        install_flowmod_on_switch_with_dpid(match_action, src_to_midbox[sw])
    ...
    for sw in range(len(midbox_to_dst)):
        ...
        if sw != len(midbox_to_dst) - 1:
            src_out_port, dst_in_port = get_ports_connecting(midbox_to_dst[sw], midbox_to_dst[sw + 1])
            match_action = build_openflow_flowmod(match_fields, src_out_port)

        else:  # last switch in the path to dest, host on the other side
            port_to_host = get_host_port(midbox_to_dst[sw])
            match_action = build_openflow_flowmod(match_fields, port_to_host)

        install_flowmod_on_switch_with_dpid(match_action, midbox_to_dst[sw])
